...
bool DiskIODriver_USBFlash::usbStartup()
{
  if (state <= DO_STARTUP)
  {
    ...
    {
    case 0x01:
      SERIAL_ECHOLNPGM("rev.01 started");
      break;
    case 0x12:
      SERIAL_ECHOLNPGM("rev.02 started");
      break;
    case 0x13:
      SERIAL_ECHOLNPGM("rev.03 started");
      break;
    default:
      SERIAL_ECHOLNPGM("started. rev unknown.");
      break;
    }
...
#define GOTO_STATE_AFTER_DELAY(STATE, DELAY) \
  do                                         \
  {                                          \
    state = STATE;                           \
    next_state_ms = millis() + DELAY;        \
  } while (0)
