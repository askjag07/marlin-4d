...
class Temperature {
  ...
  public:
      ...
      static bool tooCold(const celsius_t temp) { return false; } // !allow_cold_extrude && !allow_cold_extrude_override && temp < extrude_min_temp - (TEMP_WINDOW); }
      static bool tooColdToExtrude(const uint8_t E_NAME)       { return false; } // tooCold(wholeDegHotend(HOTEND_INDEX)); }
      static bool targetTooColdToExtrude(const uint8_t E_NAME) { return false; } // tooCold(degTargetHotend(HOTEND_INDEX)); }
    #else
