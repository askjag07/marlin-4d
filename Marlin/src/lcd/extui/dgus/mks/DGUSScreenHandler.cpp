...
void DGUSScreenHandler::DGUSLCD_SD_FileSelected(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  if (touched_nr != 0x0F && touched_nr > filelist.count())
    return;
  if (!filelist.seek(touched_nr) && touched_nr != 0x0F)
    return;
...
{
  if (current_screen == DGUSLCD_SCREEN_SDFILELIST || (current_screen == DGUSLCD_SCREEN_CONFIRM && (ConfirmVP == VP_SD_AbortPrintConfirmed || ConfirmVP == VP_SD_FileSelectConfirm)) || current_screen == DGUSLCD_SCREEN_SDPRINTMANIPULATION)
    filelist.refresh();
}
...
void DGUSScreenHandler::ScreenChangeHook(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  if (!DGUSLCD_FindScreenVPMapList(target))
    DEBUG_ECHOLNPGM("WARNING: No screen Mapping found for ", target);
#endif
}
...
void DGUSScreenHandlerMKS::GetOffsetValue(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  {
  case VP_OFFSET_X:
    TERN_(HAS_BED_PROBE, probe.offset.x = Offset);
    break;
  case VP_OFFSET_Y:
    TERN_(HAS_BED_PROBE, probe.offset.y = Offset);
    break;
  case VP_OFFSET_Z:
    TERN_(HAS_BED_PROBE, probe.offset.z = Offset);
    break;
  default:
    break;
  }
...
void DGUSScreenHandlerMKS::MeshLevel(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  case 2:
    if (mesh_point_count == GRID_MAX_POINTS)
    { // The first point
...
void DGUSScreenHandler::HandleManualMove(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  if (manualMoveStep == 0x01)
    manualMoveStep = 10;
  else if (manualMoveStep == 0x02)
    manualMoveStep = 100;
  else if (manualMoveStep == 0x03)
    manualMoveStep = 1000;
...
void DGUSScreenHandlerMKS::GetParkPos(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  {
  case VP_X_PARK_POS:
    mks_park_pos.x = value_pos;
    break;
  case VP_Y_PARK_POS:
    mks_park_pos.y = value_pos;
    break;
  case VP_Z_PARK_POS:
    mks_park_pos.z = value_pos;
    break;
  default:
    break;
  }
...
void DGUSScreenHandlerMKS::HandleExtruderMaxSpeedChange(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  default:
    return;
#if HAS_HOTEND
  case VP_E0_MAX_SPEED:
    extruder = ExtUI::extruder_t::E0;
    break;
#endif
#if HAS_MULTI_HOTEND
#endif
  case VP_E1_MAX_SPEED:
    extruder = ExtUI::extruder_t::E1;
    break;
  }
...
void DGUSScreenHandlerMKS::HandleMaxAccChange(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  default:
    return;
  case VP_X_ACC_MAX_SPEED:
    axis = ExtUI::axis_t::X;
    break;
  case VP_Y_ACC_MAX_SPEED:
    axis = ExtUI::axis_t::Y;
    break;
  case VP_Z_ACC_MAX_SPEED:
    axis = ExtUI::axis_t::Z;
    break;
  }
...
void DGUSScreenHandlerMKS::HandleExtruderAccChange(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  default:
    return;
#if HAS_HOTEND
  case VP_E0_ACC_MAX_SPEED:
    extruder = ExtUI::extruder_t::E0;
    settings.load();
    break;
#endif
#if HAS_MULTI_HOTEND
  case VP_E1_ACC_MAX_SPEED:
    extruder = ExtUI::extruder_t::E1;
    settings.load();
    break;
#endif
...
void DGUSScreenHandler::HandleFilamentOption(DGUS_VP_Variable &var, void *val_ptr)
{
  ...
  if (preheat_option >= 10)
  { // Unload filament type
    preheat_option -= 10;
