...
void MarlinUI::draw_custom_bootscreen(const uint8_t frame /*=0*/)
{
  ...
  if (frame == 0)
  {
    u8g.setColorIndex(1);
    if (top)
      u8g.drawBox(0, 0, LCD_PIXEL_WIDTH, top);
    if (left)
      u8g.drawBox(0, top, left, CUSTOM_BOOTSCREEN_BMPHEIGHT);
    if (right < LCD_PIXEL_WIDTH)
      u8g.drawBox(right, top, LCD_PIXEL_WIDTH - right, CUSTOM_BOOTSCREEN_BMPHEIGHT);
    if (bottom < LCD_PIXEL_HEIGHT)
      u8g.drawBox(0, bottom, LCD_PIXEL_WIDTH, LCD_PIXEL_HEIGHT - bottom);
  }
#endif
}
...
void MarlinUI::show_custom_bootscreen()
{
...
#endif
  {
    ...
    do
    {
      draw_custom_bootscreen(f);
    } while (u8g.nextPage());
    if (frame_time)
      safe_delay(frame_time);
  }
...
void MarlinUI::draw_marlin_bootscreen(const bool line2 /*=false*/)
{
  ...
  {
    u8g.drawBitmapP(2, 0, START_BMP_BYTEWIDTH, START_BMPHEIGHT, bitmap);
  };
...
void MarlinUI::show_marlin_bootscreen()
{
  for (uint8_t q = bootscreen_pages; q--;)
  {
    ...
    if (q)
      safe_delay((BOOTSCREEN_TIMEOUT) / bootscreen_pages);
  }
}
...
{
  if ((BOOTSCREEN_TIMEOUT) / bootscreen_pages > sofar)
    safe_delay((BOOTSCREEN_TIMEOUT) / bootscreen_pages - sofar);
}
...
#ifdef LCD_LANGUAGE_2
  case 1:
    uxg_SetUtf8Fonts(LANG_FONT_INFO(LCD_LANGUAGE_2), COUNT(LANG_FONT_INFO(LCD_LANGUAGE_2)));
    break;
#endif
#ifdef LCD_LANGUAGE_3
  case 2:
    uxg_SetUtf8Fonts(LANG_FONT_INFO(LCD_LANGUAGE_3), COUNT(LANG_FONT_INFO(LCD_LANGUAGE_3)));
    break;
#endif
#ifdef LCD_LANGUAGE_4
  case 3:
    uxg_SetUtf8Fonts(LANG_FONT_INFO(LCD_LANGUAGE_4), COUNT(LANG_FONT_INFO(LCD_LANGUAGE_4)));
    break;
#endif
#ifdef LCD_LANGUAGE_5
  case 4:
    uxg_SetUtf8Fonts(LANG_FONT_INFO(LCD_LANGUAGE_5), COUNT(LANG_FONT_INFO(LCD_LANGUAGE_5)));
    break;
#endif
  }
#else
...
static bool mark_as_selected(const uint8_t row, const bool sel)
{
  ...
  if (sel)
  {
    ...
    u8g.setColorIndex(1); // solid text
#endif
...
void MenuEditItemBase::draw_edit_screen(FSTR_P const ftpl, const char *const value /*=nullptr*/)
{
  ...
  {
    if (labellen * prop + vallen * prop + 1 > lcd_edit_width)
      extra_row = true;
  ...
  if (onpage)
    lcd_put_u8str(0, baseline, ftpl, itemIndex, itemStringC, itemStringF);
...
void MenuItem_confirm::draw_select_screen(FSTR_P const yes, FSTR_P const no, const bool yesno, FSTR_P const fpre, const char *const string /*=nullptr*/, FSTR_P const suff /*=nullptr*/)
{
  ...
  if (yes)
    draw_boxed_string(LCD_WIDTH - (utf8_strlen(yes) * (USE_WIDE_GLYPH ? 2 : 1) + 1), LCD_HEIGHT - 1, yes, yesno);
}
...
void MenuItem_sdbase::draw(const bool sel, const uint8_t row, FSTR_P const, CardReader &theCard, const bool isDir)
{
  if (mark_as_selected(row, sel))
  {
    ...
    if (isDir)
      lcd_put_lchar(LCD_STR_FOLDER[0]);
