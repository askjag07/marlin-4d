...
// CHARSET_BOOT
  ...
  const static PROGMEM byte corner[4][8] = {{B00000,
                                             B00000,
                                             B00000,
                                             B00000,
                                             B00001,
  ...
  // CHARSET_INFO
  const static PROGMEM byte progress[3][8] = {{B00000,
                                               B10000,
                                               B10000,
                                               B10000,
                                               B10000,
...
void lcd_scroll(const lcd_uint_t col, const lcd_uint_t line, FSTR_P const ftxt, const uint8_t len, const int16_t time)
{
  ...
  else
  {
    ...
    LOOP_LE_N(i, slen)
    {
      ...
      // Fill with spaces
      for (uint8_t ix = slen - i; ix < len; ++ix)
        lcd_put_lchar(' ');
      ...
      p++;
      while (!START_OF_UTF8_CHAR(pgm_read_byte(p)))
        p++;
    }
  }
}
...
static void logo_lines(FSTR_P const extra)
{
  ...
  lcd_put_lchar(indent, 0, '\x00');
  lcd_put_u8str(F("------"));
  lcd_put_lchar('\x01');
  lcd_put_u8str(indent, 1, F("|Marlin|"));
  lcd_put_u8str(extra);
  lcd_put_lchar(indent, 2, '\x02');
  lcd_put_u8str(F("------"));
  lcd_put_lchar('\x03');
}
...
void MarlinUI::draw_status_message(const bool blink)
{
  ...
  if (ELAPSED(millis(), progress_bar_ms + PROGRESS_BAR_MSG_TIME) || !has_status())
  {
    ...
    if (progress > 2)
      return draw_progress_bar(progress);
  }
  ...
  else
  {
    ...
    if (rlen < LCD_WIDTH)
    {
      ...
      if (--chars)
      { // Draw a second space if there's room
        lcd_put_lchar(' ');
        if (--chars)
        { // Draw a third space if there's room
          lcd_put_lchar(' ');
          if (--chars)
...
void MarlinUI::update_indicators()
{
  ...
  if (TERN0(HAS_HEATED_BED, thermalManager.degTargetBed() > 0))
    leds |= LED_A;
  if (TERN0(HAS_HOTEND, thermalManager.degTargetHotend(0) > 0))
    leds |= LED_B;

#if HAS_FAN
  if (TERN0(HAS_FAN0, thermalManager.fan_speed[0]) || TERN0(HAS_FAN1, thermalManager.fan_speed[1]) || TERN0(HAS_FAN2, thermalManager.fan_speed[2]) || TERN0(HAS_FAN3, thermalManager.fan_speed[3]) || TERN0(HAS_FAN4, thermalManager.fan_speed[4]) || TERN0(HAS_FAN5, thermalManager.fan_speed[5]) || TERN0(HAS_FAN6, thermalManager.fan_speed[6]) || TERN0(HAS_FAN7, thermalManager.fan_speed[7]))
    leds |= LED_C;
#endif // HAS_FAN

  if (TERN0(HAS_MULTI_HOTEND, thermalManager.degTargetHotend(1) > 0))
    leds |= LED_C;
...
#define _ZLABEL(X, Y) _LABEL('Z', X, Y)
  ...
  if (pixels_per_x_mesh_pnt >= HD44780_CHAR_WIDTH)
  ...
  if (pixels_per_y_mesh_pnt >= HD44780_CHAR_HEIGHT)
...
void add_edges_to_custom_char(custom_char &custom, const coordinate &ul, const coordinate &lr, const coordinate &brc, const uint8_t cell_location)
{
  ...
  if (ul.row == 0 && (cell_location & (TOP_LEFT | TOP_RIGHT)))
